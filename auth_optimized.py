"""
Auth module optimizado - FIX TIMEOUT LOGIN

Problema: Lazy loading de user.company causaba timeout
...
# ⭐ FUNCIÓN MEJORADA - CON EAGER LOADING
def verify_token(token: str = Depends(oauth2_scheme), db: Session = Depends(database.get_db)):
    ...
    try:
        ...
        # ⭐ EAGER LOADING - Cargar company en la misma query
...
# ⭐ FUNCIÓN OPTIMIZADA - EAGER LOADING PARA EVITAR TIMEOUT
def authenticate_user(db: Session, username: str, password: str, company_tax_id: str = None):
    """Autenticar usuario con eager loading para evitar timeout"""
